...
### 13 Techniques (absord, không tạo skill mới)
...
| 13 | **codebase-design** | **Deep module vocabulary** (Module/Interface/Depth/Seam/Adapter/Leverage/Locality) + **deletion test** ("bỏ module này thì complexity biến mất hay dời chỗ?"). → Viết parser/skill mới: thiết kế module SÂU (interface nhỏ, logic nhiều ẩn sau), 1 adapter = giả thuyết, 2 adapter = seam thật. Xem `references/codebase-design.md`. |
...
`grill-me`·`grill-with-docs`·`grilling` (interview-me đã absorb) · `implement` (incremental-implementation sâu hơn) · `research` (deep-research) · `handoff`·`writing-great-skills` (vault superset) · `resolving-merge-conflicts` (thuần code/git) · `teach` (chưa có use case training staff).
