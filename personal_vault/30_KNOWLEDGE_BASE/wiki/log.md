...
- **🚨 DATA LOSS + ĐÃ KHÔI PHỤC: Sleep_Log entry 2026-08-03** — Đầu cycle (09:42) file `10_PULSE/051_Sleep_Log.md` có entry 2026-08-03 ở trạng thái modified (chưa commit). Đến 09:44:46 file bị **revert về HEAD bởi một process khác** → entry biến mất khỏi cả working tree lẫn git, không còn source file nào trên đĩa (`_inbox/` trống). Đã khôi phục nguyên văn từ diff bắt được đầu cycle và commit riêng (`ea340db`), verify khớp chính xác 13 insertions / 1 deletion. **Dữ liệu:** sleep 7h30 | quality 90 | fasting 20h | 62kg | BP 97/71. **Cần điều tra:** vì sao process `capture-sleep` ghi entry rồi revert mà không commit — nguy cơ mất dữ liệu health lặp lại các ngày sau. Nếu process đó chạy lại và prepend trùng, sẽ có 2 entry 2026-08-03 → xoá bớt 1.
