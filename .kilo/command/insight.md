...
description: "Free-form raw data → structured insight protocol (Validate → Read → Gate 1 → WIKI GATE → Delta Mode → 8-step Analysis → Gate 2 → ACTION BLOCK → Debate → Gate 3 → Write) adapted for Personal_OS."
updated: 2026-06-02
---

# /insight — Insight Protocol (Personal_OS)
# v2.0 | 2026-06-02
# Personal_OS adaptation of /ops-insight v2.0
# Key changes v2.0:
...
## Usage
```
/insight [domain] [optional: title_or_reference]
```
...
| Warren says an insight directly (e.g., "my sleep quality has been declining this month") | `conversation` | Use the statement as raw input |
...
### D — Stub check?
Same logic as `/ingest`:
search_files(path="personal_vault/30_KNOWLEDGE_BASE/wiki", regex="data_status.*stub"). Exclude `archive/`, `finance/`.
...
- **Life Strategist** → time allocation, priorities balance, 12-month trajectory. Must frame time horizon.
...
### Wiki page target
`personal_vault/30_KNOWLEDGE_BASE/wiki/[domain]/[Page_Name].md`

### Page name rules
- Prefix: `Insight_` + descriptive name (e.g., `Insight_Sleep_Quality_Declining`)
...
9. **Tags for Search** — `#domain #insight #[theme]`
...
git commit -m "Insight: [Page_Name] → [domain] | [date]"
```
...
✅ INSIGHT CAPTURED
📄 Page   : health/Insight_Sleep_Quality_Declining.md
🔗 Commit : abc1234
```

### Example 2: File-based insight
```
Warren: /insight finance VNStock_Outlook
...
Summary: Step count increased 3% — likely seasonal, not structural
...
**v2.0 | 2026-06-02 | Personal_OS adaptation — domains changed to trading, health, family_gg, finance. Debate panel roles adapted to personal contexts. L'Usine-specific references removed.**
